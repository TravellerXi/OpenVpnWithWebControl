...
def change_passwd():
    ...
    md5credit = request.cookies.get('credit')
    if Checkmd5(md5credit,user)>0:
        return (title_setup_pc('修改密码') + '''<form action="/changepasswd" method="post">
...
def Success_login_mobile():
    ...
    if not md5credit:
        ...
        if not checkmd5:
            ...
            if Checkisadmin(user)>0:
                sourcecode = "<h3>Hello, 系统管理员: " + user + "!</h3><br><br>当前已注册用户列表>>>><br>" + Returnlist(
                    '/openvpn/users') + "<br><a href='vpn'target='_blank'>获取/刷新VPN配置文件</a><br><br><br><a href='logout' >注销</a>"
            else:
                sourcecode = "<h3>Hello,普通用户： " + user + "！</h3><br><br><a href='vpn'target='_blank'>获取/刷新VPN配置文件</a><br><br><br><a href='logout' >注销</a>"
